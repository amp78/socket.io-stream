...
Socket.prototype._write = function(id, chunk, encoding, callback) {
  ...
  this.sio.emit(exports.event + '-write', id, chunk, encoding);
  callback();
};
...
Socket.prototype._onwrite = function(id, chunk, encoding) {
  debug('write: "%s"', id);

  var stream = this.streams[id];
  if (!stream) {
    debug('invalid stream id: ' + id);
    return;
  }
  ...
  stream._onwrite(chunk, encoding);
};
